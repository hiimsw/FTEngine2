	...
	// 몬스터를 업데이트한다.
	{
		...
		if (mMonsterSpawnTimer >= 0.5f)
		{
			for (uint32_t i = 0; i < MONSTER_COUNT; ++i)
			{
				...
				monster.bulletEffectScale = { .width = 20.0f, .height = 720.0f };
				monster.hpValue = MONSTER_MAX_HP;

				DEBUG_LOG("%d", monster.isLife);
